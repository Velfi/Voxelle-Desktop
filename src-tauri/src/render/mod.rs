...
impl WgpuViewer {
    ...
    pub fn rebuild_mesh_gpu_greedy(&mut self, voxels: &[Voxel]) -> Result<MeshBounds, String> {
        ...
        let mut vtx_prefix = Vec::with_capacity(headers.len());
        let mut idx_prefix = Vec::with_capacity(headers.len());
        let mut acc_v: u32 = 0;
        let mut acc_i: u32 = 0;
        for h in &headers {
            vtx_prefix.push(acc_v);
            idx_prefix.push(acc_i);
        ...
        let bind = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: vtx_base_buf.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: idx_base_buf.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 5,
